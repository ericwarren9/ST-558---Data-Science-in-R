...
```{r combine datasets function 1}
combining_education_data <- function(data1, data2, ...){
  # Here we are going to combine our data first to get it all together
  ...
  county_data <- combined_data %>% 
    filter(grepl(pattern = ", \\w\\w", area_name)) %>%
    select(-division)
  class(county_data) <- c("county", class(county_data)) # Create the new class called county

  # Here we are going to make the non-county level data
  non_county_data <- combined_data %>% 
    filter(!grepl(pattern = ", \\w\\w", area_name)) %>%
    select(-state)
  class(non_county_data) <- c("state", class(non_county_data)) # Create the new class called state
  
  # Return our two tibbles (can only do so in list format)
  return(list(county_data, non_county_data))
}
```

We are now going to call this function `combining_education_data()` to combine the result of the two calls to the wrapper function. We will show what our data now looks like below.
```{r combining datasets 1}
combined_data <- combining_education_data(data1, data2)
combined_data
```