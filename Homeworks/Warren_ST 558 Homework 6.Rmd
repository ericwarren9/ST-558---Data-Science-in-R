...
list_sample_data <- unname(lapply(split(expand.grid(n = n, shape = shape),seq_along(expand.grid(n = n, shape = shape)[,1])), as.list))
...
results_grid <- parLapply(cluster, 1:length(list_sample_data), function(i){
  ...
  sample_data <- replicate(10000, rgamma(n = list_sample_data[[i]]$n, shape = list_sample_data[[i]]$shape, rate = 1)) # Sample data
  mu_0_gamma <- list_sample_data[[i]]$shape * 1 # Get what the mu value is for a particular shape and rate = 1
  list_of_ttests <- lapply(1:ncol(sample_data), function(i){
    t_statistic <- calculate_t_statistic(sample_data[ , i], mu_0_gamma)
    hypothesis_test(t_statistic, length(sample_data[ , i]), alpha = 0.05, direction = "two sided")
...
```{r make table for MC, dev='png'}
library(tidyverse)
# Turn lists to data frames
sample_data_combos <- expand.grid(n = n, shape = shape)
